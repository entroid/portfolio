...
- **Cero fotografía decorativa**: el sitio es 100% gráfico y tipográfico. La
  única foto real fuera del trabajo es la de perfil (navbar, y el retrato del
  hero, hoy comentado en el código); el resto de las imágenes reales son las
  capturas y mockups de producto dentro de los case studies.
...
| **Kier Studio — Agencia & Instrumentación** (featured) | Co-founder & Design Engineer          | Sitio de la agencia de diseño y desarrollo que co-fundó, hecho de punta a punta y en solitario: posicionamiento y definición de servicios, arquitectura de información, copy, UX/UI, implementación front-end, deploy y analítica. Sin brief de cliente, sin stakeholders y sin equipo a quien hacer handoff — el caso cuenta el método completo, de la estrategia al código medido, no a un archivo de Figma.                                                                                                                                                                                      |
...
- **Tab "Prompting"** — el más reciente y el más opinado: sostiene que el
  _prompt engineering_ como oficio se está diluyendo, porque los modelos ya
  no responden al prompt elaborado. Lo que un buen prompt **todavía**
  necesita: la tarea, los límites (_guardrails_), la definición de _done_ y
  dejar correr sin micro-gestionar. Incluye un ejemplo real —un pedido de la
  migración de Hard Rock reescrito a su forma actual— y cierra con que
  escribir ese prompt tomó cuatro minutos, pero saber qué poner bajo
  "done when" tomó quince años.
...
| CI                  | GitHub Actions: lint → typecheck → test → build en cada PR y en cada push a `main`                                                                                |
